...
PylintRepoAllowlistInfo = provider(
    doc = "Provides a list of allowed canonical repository names for Pylint.",
    fields = {
        "allowed_canonical_names": "list of strings",
    },
)

def _pylint_repo_allowlist_impl(ctx):
    return [PylintRepoAllowlistInfo(allowed_canonical_names = ctx.attr.allowed_canonical_names)]

pylint_repo_allowlist = rule(
    doc = """Stores a list of allowed canonical repository names for Pylint.

Bzlmod mangles repository names (canonical names) to ensure uniqueness, while
users typically use apparent names in their BUILD files. Because an aspect
defined in an external repository (like Pigweed) cannot resolve apparent names
of downstream repositories using `Label()`, downstream projects must use a macro
in their own repository to resolve apparent names to canonical names and pass
them to this rule.

Example:

In downstream `defs.bzl`:
```python
load("@pigweed//pw_build:pw_pylint.bzl", "pylint_repo_allowlist")

def pylint_allowlist(name, repos):
    canonical_names = []
    for r in repos:
        lbl = Label("@" + r + "//:__pkg__")
        if lbl.workspace_name:
            canonical_names.append(lbl.workspace_name)

    pylint_repo_allowlist(
        name = name,
        allowed_canonical_names = canonical_names,
    )
```

In downstream `BUILD.bazel`:
```python
load("//:defs.bzl", "pylint_allowlist")

pylint_allowlist(
    name = "pylint_allowed_repos",
    repos = ["repo1", "repo2", "repo3"],
)
```
""",
    implementation = _pylint_repo_allowlist_impl,
    attrs = {
        "allowed_canonical_names": attr.string_list(
            doc = "List of resolved canonical repository names.",
            mandatory = True,
        ),
    },
)
...
def _pylint_aspect_impl(target, ctx):
    ...
    allowed_target = ctx.attr._allowed_repos_target
    allowed_canonical_names = []
    if allowed_target and PylintRepoAllowlistInfo in allowed_target:
        allowed_canonical_names = allowed_target[PylintRepoAllowlistInfo].allowed_canonical_names

    # Skip linting for files from external workspaces (unless allowed) or generated by rules_python.
    direct_srcs = []
    for src in _filter_out_rules_python_generated_sources(ctx.rule.files.srcs, ctx):
        if not src.owner.workspace_root:
            direct_srcs.append(src)
        elif src.owner.workspace_name in allowed_canonical_names:
            direct_srcs.append(src)
...
pylint_aspect = aspect(
    ...
    attr_aspects = [],
    attrs = {
        "_allowed_repos_target": attr.label(
            default = Label("@pigweed//pw_build:pylint_allowed_repositories_target"),
        ),
        "_pylint": attr.label(
