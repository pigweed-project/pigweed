...
class SearchTest(unittest.TestCase):
    ...
    def test_search_excerpt_excludes_toc_fab(self):
        """Verifies that search result excerpts for 'pw_async2'
        exclude 'toc', ensuring #pw-toc-fab button is excluded."""
        excerpt_text = self._get_first_search_excerpt("pw_async2")
        self.assertTrue(
            excerpt_text,
            "Expected search result excerpt to be non-empty",
        )
        self.assertNotIn(
            "toc",
            excerpt_text,
            f"First search result excerpt should not contain 'toc', got: {excerpt_text}",
        )

    def test_search_excerpt_excludes_collapse_sidebar(self):
        """Verifies that search result excerpts for 'pw_async2'
        exclude 'Collapse Sidebar', ensuring .pst-collapse-sidebar-label is excluded.
        """
        excerpt_text = self._get_first_search_excerpt("pw_async2")
        self.assertTrue(
            excerpt_text,
        ...
        self.assertNotIn(
            "Collapse Sidebar",
            excerpt_text,
            f"First search result excerpt should not contain 'Collapse Sidebar', got: {excerpt_text}",
        )

    def test_search_excerpt_excludes_expand_sidebar(self):
        """Verifies that search result excerpts for 'pw_async2'
        exclude 'Expand Sidebar', ensuring .pst-expand-sidebar-label is excluded.
        """
        excerpt_text = self._get_first_search_excerpt("pw_async2")
        self.assertTrue(
            excerpt_text,
        ...
        self.assertNotIn(
            "Expand Sidebar",
            excerpt_text,
            f"First search result excerpt should not contain 'Expand Sidebar', got: {excerpt_text}",
        )

    def test_search_excerpt_excludes_back_to_top(self):
        """Verifies that search result excerpts for 'pw_async2'
        exclude 'Back to top', ensuring #pst-back-to-top button is excluded."""
        excerpt_text = self._get_first_search_excerpt("pw_async2")
        self.assertTrue(
            excerpt_text,
            "Expected search result excerpt to be non-empty",
        ...
        self.assertNotIn(
            "Back to top",
            excerpt_text,
            f"First search result excerpt should not contain 'Back to top', got: {excerpt_text}",
        )

    def test_search_excerpt_excludes_doxygen_footer(self):
        """Verifies that search result excerpts for 'Heap allocate space for tasks'
        exclude 'Generated by', ensuring Doxygen .footer is excluded."""
        excerpt_text = self._get_first_search_excerpt(
        ...
        self.assertNotIn(
            "Generated by",
            excerpt_text,
            f"First search result excerpt should not contain 'Generated by', got: {excerpt_text}",
        )

    def test_search_excerpt_excludes_doxygen_navrow1(self):
        """Verifies that search result excerpts for 'Heap allocate space for tasks'
        exclude 'Modules', ensuring Doxygen site navigation #navrow1 is excluded.
        """
        excerpt_text = self._get_first_search_excerpt(
            "Heap allocate space for tasks"
        )
        ...
        self.assertNotIn(
            "Modules",
            excerpt_text,
            f"First search result excerpt should not contain 'Modules', got: {excerpt_text}",
        )

    def test_search_excerpt_excludes_doxygen_loading(self):
        """Verifies that search result excerpts for 'Heap allocate space for tasks'
        exclude 'Loading...', ensuring Doxygen #Loading status UI is excluded.
        """
        excerpt_text = self._get_first_search_excerpt(
            "Heap allocate space for tasks"
        )
        ...
        self.assertNotIn(
            "Loading...",
            excerpt_text,
            f"First search result excerpt should not contain 'Loading...', got: {excerpt_text}",
        )

    def test_search_excerpt_excludes_doxygen_searching(self):
        """Verifies that search result excerpts for 'EpollDispatcher Member List'
        exclude 'Searching...', ensuring Doxygen #Searching status UI is excluded.
        """
        excerpt_text = self._get_first_search_excerpt(
            "EpollDispatcher Member List"
        )
        self.assertTrue(
            excerpt_text,
            "Expected search result excerpt to be non-empty",
        ...
        self.assertNotIn(
            "Searching...",
            excerpt_text,
            f"First search result excerpt should not contain 'Searching...', got: {excerpt_text}",
        )

    def test_search_excerpt_excludes_doxygen_no_matches(self):
        """Verifies that search result excerpts for 'EpollDispatcher Member List'
        exclude 'No Matches', ensuring Doxygen #NoMatches status UI is excluded.
        """
        excerpt_text = self._get_first_search_excerpt(
            "EpollDispatcher Member List"
        )
