...
compile_error!("SMP spinlock requires hardware atomic compare-and-swap support");
...
use super::common::{CortexMSpinLockGuard, InterruptGuard};
...
impl BareSpinLockTrait for BareSpinLock {
    ...
    fn try_lock(&self) -> Option<Self::Guard<'_>> {
        ...
        if self
            ...
            .is_ok()
        {
            Some(CortexMSpinLockGuard::new(guard, self))
        } else {
            None
        }
    }
    ...
    fn lock(&self) -> Self::Guard<'_> {
        ...
        // retry loop: any spurious LL/SC (`ldaexb`/`strexb`) failure is
        // retried by the outer `while` loop without emitting a nested loop.
        while self
            .is_locked
        ...
        CortexMSpinLockGuard::new(guard, self)
    }
