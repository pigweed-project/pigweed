...
class TestFindCoreOwners(unittest.TestCase):
    ...
    def test_find_nonlsc_revision(self, mock_run):
        """Tests finding non-LSC revision from git log."""
        mock_run.side_effect = self._mock_subprocess_run

        # Should skip Commit 3 (LSC) and return Commit 2 (non-LSC)
        finder = CoreOwnerFinder(repo=ReadOnlyGitWorkspace(self.repo_path))
    ...
    def test_all_lsc_revisions_returns_none(self, mock_run):
        """Tests that all-LSC revisions returns None."""
        mock_run.side_effect = self._mock_subprocess_run
    ...
    def test_find_core_owner_with_lines(self, mock_run, mock_cores):
        """Tests finding core owner for specific line numbers."""
        mock_run.side_effect = self._mock_subprocess_run
    ...
    def test_find_core_owner_whole_file(self, mock_run, mock_cores):
        """Tests finding core owner for a whole file."""
        mock_run.side_effect = self._mock_subprocess_run
        mock_cores.return_value = {'user1@google.com', 'user2@google.com'}
        finder = CoreOwnerFinder(repo=ReadOnlyGitWorkspace(self.repo_path))
    ...
    @patch.dict(os.environ, {'BUILD_WORKING_DIRECTORY': '/some/other/dir'})
    @patch('pw_fortifier.find_core_owners.CoreOwnerFinder.core_members')
    @patch('subprocess.run')
    def test_find_core_owner_relative_path_with_build_working_dir(
        self, mock_run, mock_cores
    ):
        """Tests finding owner with relative path when env is set."""
        mock_run.side_effect = self._mock_subprocess_run
        mock_cores.return_value = {'user1@google.com', 'user2@google.com'}
        finder = CoreOwnerFinder(repo=ReadOnlyGitWorkspace(self.repo_path))
        finder.add('module1/file.txt')
        author = finder.find()
        self.assertEqual(author, 'user2@google.com')
    ...
    def test_find_core_owner_any_owner(self, mock_run, mock_cores):
        """Tests finding any owner when any_owner is True."""
        mock_run.side_effect = self._mock_subprocess_run
        mock_cores.return_value = {'user1@google.com', 'user2@google.com'}
        finder = CoreOwnerFinder(repo=ReadOnlyGitWorkspace(self.repo_path))
        finder.add(os.path.join(self.repo_path, 'module1/file.txt'))
    ...
    @patch('pw_fortifier.find_core_owners.CoreOwnerFinder.core_members')
    def test_find_file_outside_repo_raises(self, mock_cores):
        """Tests that file outside repository raises ValueError."""
        mock_cores.return_value = {'user1@google.com'}
        finder = CoreOwnerFinder(repo=ReadOnlyGitWorkspace(self.repo_path))
        finder.add('/tmp/outside_repo.txt')
    ...
    def test_find_local_owner_fallback(
        ...
        """Tests fallback to local OWNERS file."""
    ...
    def test_find_local_owner_fallback_any_owner(
        ...
        """Tests fallback to local OWNERS file with any_owner."""
        mock_run.return_value = subprocess.CompletedProcess(
            args=[], returncode=0, stdout='', stderr=''
        )
        mock_cores.return_value = {'core_user@google.com'}
    ...
    def test_parse_owners_file(self):
        """Tests parsing owners from an OWNERS file."""
    ...
    def test_core_members(self):
        """Tests extracting core team members from root OWNERS file."""
        mock_content = (
            '# Some header\n'
    ...
    def test_core_members_no_file(self):
        """Tests FileNotFoundError when root OWNERS file is missing."""
    ...
    def test_parse_snippet_arg(self):
        """Tests parsing file and line number snippet arguments."""
    ...
    def test_main(self, mock_finder_class):
        """Tests CLI main function with snippet arguments."""
        mock_finder = MagicMock()
        mock_finder.find.return_value = 'assignee@google.com'
        ...
        self.assertIn('file.cc:10-20: assignee@google.com', output)
        self.assertIn('file2.cc: assignee@google.com', output)

        self.assertEqual(mock_finder_class.call_count, 2)
        called_repo = mock_finder_class.call_args[1]['repo']
        self.assertEqual(called_repo.project_dir, Path('.').resolve())
    ...
    def test_main_any_owner(self, mock_finder_class):
        """Tests CLI main function with --any flag."""
        mock_finder = MagicMock()
        mock_finder.find.return_value = 'assignee@google.com'
        mock_finder_class.return_value = mock_finder
        ...
        output = mock_stdout.getvalue()
        self.assertIn('file.cc: assignee@google.com', output)
    ...
    def test_main_with_root(self, mock_finder_class):
        """Tests CLI main function with custom root flag."""
        mock_finder = MagicMock()
        mock_finder.find.return_value = 'assignee@google.com'
        mock_finder_class.return_value = mock_finder
        ...
        called_repo = mock_finder_class.call_args[1]['repo']
        self.assertEqual(
            called_repo.project_dir, Path('/tmp/mock_root').resolve()
        )
